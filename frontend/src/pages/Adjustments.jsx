...
import { Plus, LayoutList, Layers, RefreshCw } from 'lucide-react';
import MainLayout from '../components/layout/MainLayout';
import { useOperations } from '../hooks/useOperations';
import { useAuth } from '../context/AuthContext';
import AdjustmentTable from '../components/Operations/AdjustmentTable';
...
const Adjustments = () => {
  ...
  const { operations: adjustments, isLoading, fetchOperations, createOperation, validateOperation, updateOperation } = useOperations('adjustments');
  ...
  return (
    <MainLayout>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <RefreshCw className="text-amber-500" /> Inventory Adjustments
            </h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Fix stock tracking mismatches based on physical counts</p>
          </div>
        ...
        {/* Content */}
        {isLoading ? (
          <div className="flex justify-center p-20"><div className="w-8 h-8 border-4 border-amber-500 border-t-transparent rounded-full animate-spin"></div></div>
        ) : viewMode === 'list' ? (
          <AdjustmentTable adjustments={adjustments} isManager={isManager} onValidate={validateOperation} />
        ) : (
          <OperationKanban 
