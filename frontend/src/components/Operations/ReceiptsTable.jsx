        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {receipts.length > 0 ? receipts.map((receipt, i) => (
              ...
              >
                <td className="py-4 px-6 font-bold text-slate-900 dark:text-white">{receipt.reference}</td>
                <td className="py-4 px-6 text-sm text-slate-600 dark:text-slate-400">{receipt.supplier}</td>
                <td className="py-4 px-6 text-sm text-slate-600 dark:text-slate-400">{receipt.warehouse}</td>
                ...
                <td className="py-4 px-6 text-center">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold uppercase ${getStatusBadge(receipt.status)}`}>
                    {receipt.status}
                  </span>
                </td>
                <td className="py-4 px-6 text-center">
                  <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {isManager && receipt.status !== 'Done' && (
                      <>
                        <button onClick={() => onValidate(receipt._id)} className="p-1.5 text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-lg">
                          <CheckCircle size={18} />
                        </button>
                        <button onClick={() => onEdit(receipt)} className="p-1.5 text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 rounded-lg">
                          <Edit2 size={18} />
                        </button>
                      </>
                    )}
                    {isManager && receipt.status !== 'Done' && onDelete && (
                      <button onClick={() => onDelete(receipt._id)} className="p-1.5 text-slate-400 hover:text-red-600 dark:hover:text-red-400 rounded-lg">
                        <Trash2 size={18} />
                      </button>
                    )}
                  </div>
