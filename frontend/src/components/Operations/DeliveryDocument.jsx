...
const DeliveryDocument = forwardRef(({ data }, ref) => {
  if (!data) return null;

  const { reference, customer, warehouse, scheduledDate, status, products, createdBy } = data;
    ...
    <div ref={ref} className="p-12 bg-white text-slate-900 font-sans min-h-[11in] flex flex-col">
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-200 pb-8 mb-8">
        ...
        <div className="text-right">
          <h2 className="text-3xl font-black text-slate-300 uppercase leading-none mb-1">Delivery Order</h2>
      ...
      <div className="grid grid-cols-2 gap-12 mb-12">
        <div className="space-y-4">
          <div>
            <label className="text-[10px] font-bold uppercase text-slate-400 block mb-1">Customer / Consignee</label>
            <p className="text-lg font-bold text-slate-800">{customer}</p>
          </div>
          <div>
            <label className="text-[10px] font-bold uppercase text-slate-400 block mb-1">Shipping Warehouse</label>
            <p className="text-base font-semibold text-slate-700">{warehouse}</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-6">
          <div>
            <label className="text-[10px] font-bold uppercase text-slate-400 block mb-1">Shipment Date</label>
          ...
          <tbody className="divide-y divide-slate-100">
            {products.map((item, idx) => (
              <tr key={idx} className="text-sm">
                ...
                <td className="py-4 px-4 text-slate-600">{warehouse} / Storage</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="grid grid-cols-2 gap-24">
          <div className="space-y-4">
            <p className="text-xs text-slate-400 italic">
              "This delivery order is subject to terms and conditions. Please verify all items upon receipt."
            </p>
            <div className="pt-8">
          ...
          <div className="flex flex-col items-center justify-end">
            ...
            <p className="text-xs font-bold uppercase tracking-widest text-slate-900">Receiver's Signature</p>
          </div>
        </div>
      </div>
...
DeliveryDocument.displayName = 'DeliveryDocument';
